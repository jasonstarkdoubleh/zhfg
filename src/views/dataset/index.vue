    ...
    <div v-show="dataSet">
      ...
      <div style="margin-top: 10px;display: flex;background: #ffffff;padding: 10px">
        <div style="width: 25%;min-height: 500px;margin-right: 10px">
          ...
          <div
            style="height: 470px;overflow: scroll;border: 1px solid #cccccc;overflow-x: hidden;padding: 10px"
          >
            <div style="padding: 5px" v-for="(item, index) in dataName" :key="index">
              <el-radio
                v-model="radioValue"
                :label="item"
                @change="handleGetTable"
              >{{ item.dataSetName }}</el-radio>
            </div>
          </div>
        </div>
        ...
        <div style="width: 75%;min-height: 500px">
          ...
          <div style="height: 470px;overflow-y: scroll">
            <jtable :tableData="tableData" :columnData="columnData" :pageShow="false"></jtable>
          </div>
        </div>
      </div>
    </div>
        ...
        <el-form ref="form" :model="form" label-width="120px">
          ...
          <el-form-item label="数据集类型">
            <el-select v-model="form.dataSetType" :disabled="disabled">
              <el-option
                v-for="(item, index) in codeNameOptions"
                :key="index"
                :label="item.codeName"
                :value="item.codeId"
              ></el-option>
...
import {
  ...
  tree,
  macTree,
  getIndexInfoByDataSetId,
  pssdatasetinfoUpdate
} from "@/api/manager";
export default {
  data() {
    return {
  ...
  watch: {
          ...
          for (let i in this.macValue) {
            if (this.macValue[i].subList[0].indexType === val[j]) {
              for (let m in this.macValue[i].subList) {
                ...
                this.macNameOptions[num].label = this.macValue[i].subList[
                  m
                ].indexName;
                this.macNameOptions[num].value = this.macValue[i].subList[
                  m
                ].indexId;
                num++;
              }
      ...
      if (val) {
        for (let i in this.getCommTypeData.commLevelCode_1) {
          if (this.getCommTypeData.commLevelCode_1[i].parentCode === val) {
            ...
            this.goodsClassOptions[
              num
            ].value = this.getCommTypeData.commLevelCode_1[i].commId;
            this.goodsClassOptions[
              num
            ].label = this.getCommTypeData.commLevelCode_1[i].commName;
            num++;
          }
        }
        this.createCount[this.createNum].goodsClassOptions = this.createCount[
          this.createNum
        ].goodsClassOptions.concat(this.goodsClassOptions);
      }
    },
      ...
      if (val) {
        for (let i in this.getCommTypeData.commLevelCode_2) {
          if (this.getCommTypeData.commLevelCode_2[i].parentCode === val) {
            ...
            this.goodsNameOptions[
              num
            ].value = this.getCommTypeData.commLevelCode_2[i].commId;
            this.goodsNameOptions[
              num
            ].label = this.getCommTypeData.commLevelCode_2[i].commName;
            num++;
          }
        }
        this.createCount[this.createNum].goodsNameOptions = this.createCount[
          this.createNum
        ].goodsNameOptions.concat(this.goodsNameOptions);
      }
    },
      ...
      if (val) {
        for (let i in this.getCommTypeData.commLevelCode_3) {
          if (this.getCommTypeData.commLevelCode_3[i].parentCode === val) {
            ...
            this.goodsNameOptionsItems[
              num
            ].value = this.getCommTypeData.commLevelCode_3[i].commId;
            this.goodsNameOptionsItems[
              num
            ].label = this.getCommTypeData.commLevelCode_3[i].commName;
            num++;
          }
        }
        this.createCount[
          this.createNum
        ].goodsNameOptionsItems = this.createCount[
          this.createNum
        ].goodsNameOptionsItems.concat(this.goodsNameOptionsItems);
      }
    },
    ...
    commLevelCode_3(val) {
      if (val) {
        tree(val).then(res => {
          ...
          this.createCount[this.createNum].subItemOptions = this.createCount[
            this.createNum
          ].subItemOptions.concat(this.subItemOptions);
        });
      } else {
        this.createCount[this.createNum].subItem = [];
        this.createCount[this.createNum].subItemOptions = [];
    ...
    subItem(val) {
      ...
      if (val.length > 0) {
          ...
          for (let i in this.subData) {
            if (this.subData[i].subList[0].indexType === val[j]) {
              for (let m in this.subData[i].subList) {
                ...
                this.subItemSupOptions[num].label = this.subData[i].subList[
                  m
                ].indexName;
                this.subItemSupOptions[num].value = this.subData[i].subList[
                  m
                ].indexId;
                num++;
              }
        ...
        this.createCount[this.createNum].subItemSupOptions = this.createCount[
          this.createNum
        ].subItemSupOptions.concat(this.subItemSupOptions);
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    ...mapActions(["getDataSetList", "getCommType"]),
    handleGetTable() {
    ...
    updateData() {
      ...
      let fun = () => {
        if (Object.keys(this.updata.commIndevalPath).length > key) {
          ...
          this.createCount[i].commLevelCode_0 = this.updata.commIndevalPath[
            key
          ][0];
          this.$nextTick(() => {
            this.createCount[i].commLevelCode_1 = this.updata.commIndevalPath[
              key
            ][1];
            this.$nextTick(() => {
              this.createCount[i].commLevelCode_2 = this.updata.commIndevalPath[
                key
              ][2];
              this.$nextTick(() => {
                this.createCount[
                  i
                ].commLevelCode_3 = this.updata.commIndevalPath[key][3];
                this.$nextTick(() => {
                  tree(this.createCount[i].commLevelCode_3).then(res => {
                    this.createCount[i].subItem = this.updata.commIndevalPath[
                      key
                    ][4];
                    this.$nextTick(() => {
                      this.createCount[
                        i
                      ].subItemSup = this.updata.commIndevalPath[key][5];
                      i++;
                      key++;
                      fun();
                    });
    ...
    onSubmit() {
      ...
      for (; m < this.createNum + 1; m++) {
        this.form.commIndevalPath[m] = [
          this.createCount[m].commLevelCode_0,
          this.createCount[m].commLevelCode_1,
          this.createCount[m].commLevelCode_2,
          this.createCount[m].commLevelCode_3,
          this.createCount[m].subItem,
          this.createCount[m].subItemSup
        ];
      }
    ...
    handleListAll() {
      this.getDataSetList().then(res => {
        ...
        for (let i in this.dataName) {
          ...
          this.dataName[i].commIndevalPath = JSON.parse(
            this.dataName[i].commIndevalPath
          );
          this.dataName[i].macroIndevalPath = JSON.parse(
            this.dataName[i].macroIndevalPath
          );
        }
      });
      //查询商品类型
      this.getCommType().then(res => {
        ...
        for (let i in this.getCommTypeData.commLevelCode_0) {
          ...
          this.goodsTypeOptions[i].value = this.getCommTypeData.commLevelCode_0[
            i
          ].commId;
          this.goodsTypeOptions[i].label = this.getCommTypeData.commLevelCode_0[
            i
          ].commName;
        }
